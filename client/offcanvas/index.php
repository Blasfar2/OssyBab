...
$query = "SELECT * FROM cartitems c JOIN products p ON c.ProductID = p.ProductID WHERE c.UserID = $adminId";
    ...
    <div class="offcanvas-body p-0">
        ...
        <div id="cart-items">
            <?php foreach ($cartItems as $item): ?>
                <div class='row align-items-center cart-item' style='justify-content:center;flex-wrap: nowrap; '
                    data-id='<?= $item['CartItemID'] ?>' data-price='<?= $item['Price'] ?>'>
                            ...
                            <div class='row gap-1' style='justify-content:center ; align-items: baseline'>
                                ...
                                <div class='d-flex justify-content-center gap-3'>
                                    ...
                                    <form method='post'>
                                        ...
                                        <button type='submit' class='btn btn-danger' style="width:5rem"
                                            name='deletCart'>del</button>
                                    </form>
                                </div>
                            </div>
                        </div>
        ...
        <div id="cart-total" class="mt-3 sticky-bottom bg-outline-primary">
            ...
            <form method='post'>
            <input type='hidden' name='Cart_user_id' value='<?= $adminId ?>'>
                <button type='submit' name="buyAllinCart" class='btn btn-primary' style="width:5rem">buy</button>
                <button type='submit' name="deleteAllinCart" class='btn btn-danger' style="width:5rem">all del</button>
            </form>
        </div>
    </div>
</div>